    ...
    methods: {
      ...
      loadIamgeObject (container) {
        ...
        if (containerH/containerW >= imgH/imgW) {
          ...
          this.initialImageHeight = imgH * ratio
          ...
          this.initialImageHeight = containerH
          this.initBgCanvas(container, containerW, containerH, this.img.object);
          this.initCanvas(container, containerW, containerH);
          this.initMousePointerCanvas(container, containerW, containerH);
        }
      ...
      loadDicomImageToCanvas (uploadedFile) {
        ...
        this.cornerStoneCanvas = document.getElementsByClassName('cornerstone-canvas')[0]
        this.cornerStoneCanvas.style.position = 'absolute'
        this.cornerStoneCanvas.style.visibility = 'hidden'
...
//          const viewport = cornerstone.getDefaultViewportForImage(element, image);
          ...
          setTimeout(() => {
            let croppedCanvas
            if (containerH/containerW >= imgH/imgW) {
              croppedCanvas = this.cropCanvas(this.cornerStoneCanvas, 0, this.cornerStoneCanvas.height/2 - this.initialImageHeight/2, this.initialImageWidth, this.initialImageHeight)
            } else {
              croppedCanvas = this.cropCanvas(this.cornerStoneCanvas, this.cornerStoneCanvas.width/2 - this.initialImageWidth/2, 0, this.initialImageWidth, this.initialImageHeight)
            }
          ...
          let containerW = this.cornerStoneCanvas.getBoundingClientRect().width
          let containerH = this.cornerStoneCanvas.getBoundingClientRect().height
      ...
      initView () {
        ...
        if (this.bgImageCanvas) {
          this.container.removeChild(this.bgImageCanvas.node)
        }
        if (this.cornerStoneCanvas) {
          cornerstone.disable(this.container)
        }

        this.img = {}
        this.cornerStoneCanvas = null
        this.canvas = null
...
//        canvas.context.imageSmoothingEnabled = false
      ...
      initBgCanvas (container, width, height, bgImage) {
        var bgImageCanvas = this.createCanvas(container, width, height);
        this.bgImageCanvas = bgImageCanvas

        this.bgImageCanvas.context.clearRect(0,0,this.bgImageCanvas.width, this.bgImageCanvas.height);
        this.bgImageCanvas.context.drawImage(bgImage, 0,0, this.initialImageWidth, this.initialImageHeight)
      },
      ...
      addCanvasEvents (canvas, ctx) {
        let self = this // self == Vue.js instance
        ...
        canvas.node.onmousemove = function(e) {
          ...
          if (self.isMouseRight) {
            ...
            self.bgImageCanvas.node.style.filter = 'contrast(' + self.imageContrast + '%' +')'
            return
          }
        ...
        canvas.node.onmouseup = function(e) {
          ...
          // put tempDrawingPaths data into drawingPaths
      ...
      drawAfterPan (deltaX, deltaY) {
        ...
        this.bgImageCanvas.context.clearRect(0,0, this.bgImageCanvas.node.width, this.bgImageCanvas.node.height)
        this.mousePointerCanvas.context.clearRect(0,0, this.bgImageCanvas.node.width, this.bgImageCanvas.node.height)

        // these will be our new x,y position to move the image.
        this.bgImageCanvas.context.drawImage(this.img.object,  deltaX, deltaY, this.initialImageWidth, this.initialImageHeight);
...
//        }
      ...
      drawAfterZoom (zoomedX, zoomedY) {
        this.initialImageHeight = this.img.height / this.img.width * this.initialImageWidth;
        ...
        // these will be our new x,y position to move the image.
        this.bgImageCanvas.context.drawImage(this.img.object, zoomedX, zoomedY, this.initialImageWidth, this.initialImageHeight);
        for (let i=0; i<this.drawingPaths.length; i++) {
          let aPaths = this.drawingPaths[i]
          for (let j = 0; j < aPaths.length; j++) {
            let aPath = aPaths[j]
